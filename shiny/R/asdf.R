...
server<-function(input, output){
  ...
  output$plot3 <- renderPlot({
    comparison<-filter(dat, char=="experimental")
    ...
    stride<-c(monthly[,input$month3], monthly[,input$month4])
    mont<-data.frame(month,stride)
    k<-ggplot(mont, aes(x=month, y=stride)) + geom_bar(stat="identity", postiion="dodge", fill="light steel blue", width=0.1)+theme_bw()
    print(k)
  })
...
ui<-navbarPage("환자분석툴",
               ...
               tabPanel("개인 걸음수",
                        ...
                        sidebarPanel(
                          ...
                          selectInput("names", "Name : ", choices=dat$name)
                        ),
