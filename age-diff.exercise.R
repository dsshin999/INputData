...
ggplot(data=total, aes(x=age)) + #가로축을 나이대로
  geom_bar(width = 3, stat="identity", position="dodge", aes(y=diff.st, fill=char)) + #바의 넓이는 1.5, 이후 데이터는 이전 데이터보다 오른쪽에 위치하도록 2만큼 움직임
  ylab("difference of exercising") + #y축 이름 정함
  theme_bw() #뒷배경을 하얗게

ggplot(data=total, aes(x=age)) + #가로축을 나이대로
  geom_bar(width = 3, stat="identity", position="dodge", aes(y=diff.eq, fill=char)) + #바의 넓이는 1.5, 이후 데이터는 이전 데이터보다 오른쪽에 위치하도록 2만큼 움직임
  ylab("difference of EQ5D Value") + #y축 이름 정함
  theme_bw() #뒷배경을 하얗게

ggplot(data=total, aes(x=age)) + #가로축을 나이대로
  geom_bar(width = 3, stat="identity", position="dodge", aes(y=diff.mr, fill=char)) + #바의 넓이는 1.5, 이후 데이터는 이전 데이터보다 오른쪽에 위치하도록 2만큼 움직임
  ylab("difference of SF-36 MR value") + #y축 이름 정함
  theme_bw() #뒷배경을 하얗게
